...
媒體 `label` 優先於自動取得的標題；只有 label 留空且 autoTitle: true 才查詢 YouTube。固定聚會日期請設定 variables.date（例如字串 `2026/9/20`）。服事表連續 rows 可各加 `group: 樂團`，顯示共同群組標記；role 靠右對齊。QR 可寫成 `qr: { src: images/qr.png, size: 21 }`，size 是畫面寬度百分比（8–30）。歌詞依各首最寬文字估算統一字級，底框寬 90%；長文字最大字級為畫面寬度的 3.5%。英文歌名以每個單字首字母放大的小型大寫呈現。

### YouTube 提前循環與柔和音量

在 media 內設定以下秒數（只在 loop: true 時生效）：

```yaml
loop: true
loopTrimEnd: 20  # 在影片結束前 20 秒回頭
loopFade: 1.5    # 回頭前淡出 1.5 秒，回頭後淡入 1.5 秒
# loopStart: 0
# loopEnd: 240   # 可指定確切結束秒數，優先於 loopTrimEnd
```

回頭使用同一播放器，不開第二個 YouTube 串流。跳轉時先靜音，等實際播放位置回到開頭才漸強；緩衝期間不提前增加音量。這不是無縫 crossfade，網路可能造成短暫間隔。片尾內嵌宣傳或結束畫面須調整結束秒數避開；無法保證消除 YouTube 平台廣告。影片長度不足裁切秒數時不啟用提前循環。
