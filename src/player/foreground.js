...
export class ForegroundManager {
  ...
  render(item,active,cue=false){
    ...
    if(item.type==='image'){
      ...
      for(const column of item.columns){const panel=document.createElement('section');panel.className='roster-column';for(const row of column.rows){const entry=document.createElement('div');entry.className='roster-row';const role=document.createElement('span');role.textContent=row.role;const name=document.createElement('strong');name.textContent=row.name;entry.append(role,name);panel.append(entry);}columns.append(panel);}scene.append(columns);
      ...
      for(const block of blocks){const node=document.createElement(item.layout==='reading'?'div':'p');node.className=`block ${block.kind||'text'}`;if(item.layout==='reading'){node.classList.add('reading-list');for(const row of readingRows(block.text)){const line=document.createElement('div');line.className='reading-row';const number=document.createElement('span');number.className='reading-number';number.textContent=row.number;const text=document.createElement('span');text.className='reading-copy';text.textContent=row.text;line.append(number,text);node.append(line);}}else node.textContent=block.text;content.append(node);}scene.append(content);
    ...
    if(item.qr){scene.classList.add('has-qr');const qr=document.createElement('img');qr.className='slide-qr';qr.src=typeof item.qr==='string'?item.qr:item.qr.src;qr.alt='QR code';scene.append(qr);}
